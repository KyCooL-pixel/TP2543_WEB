  ...
  <center>
    ...
    <form action="orders.php" method="post">
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_staffs_a189479");
        $stmt->execute();
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a189479");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <table border="1">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a189479, tbl_staffs_a189479, tbl_customers_a189479 WHERE ";
        $sql = $sql."tbl_orders_a189479.fld_staff_num = tbl_staffs_a189479.fld_staff_num and ";
        $sql = $sql."tbl_orders_a189479.fld_customer_num = tbl_customers_a189479.fld_customer_num";
